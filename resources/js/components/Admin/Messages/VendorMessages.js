...
class VendorMessages extends Component {
    ...
    get_new_msgs(){
        ...
        Axios.post('/api/get_vendor_chats',senderdata).then(res=>{
        this.setState({
            chats:res.data
        })
        if(res.data.length > 0){
            Axios.post('/api/get_vendor_chat_messages',{chat_id:res.data[0].id}).then(res=>{
    ...
    componentWillMount(){
      setInterval(() => {
        Axios.post('/api/get_vendor_chats').then(res=>{
            ...
            if(res.data.length > 0){
                Axios.post('/api/get_vendor_chat_messages',{chat_id:this.state.active_chat == 0 ? res.data[0].id : this.state.active_chat}).then(res=>{
                    this.set_scroll();
                    console.log(Request)
                   if(res.data.status == 200){
    ...
    send_message(e){
        ...
        let newmsg = {
            ...
            sender:'a',
            vendor_id:this.state.chat.vendor_id
        }
        array.push(newmsg);
        this.setState({
            messages:array
        ...
        //   let recipient = this.state.chat.vendor_id;
    ...
    change_active_chat(chat){
        ...
        Axios.post('/api/get_vendor_chat_messages',{chat_id:chat.id}).then(res=>{
            if(res.data.status == 200){
             this.setState({
                 messages:res.data.messages,
                 active_chat:chat.id,
        ...
        return (
            ...
            <div className="row">
                ...
                <div className="panel panel-bordered">
                    ...
                    <h3 className="panel-title">Vendor Messages</h3>
                    </div>
                                                ...
                                                <div onClick={this.change_active_chat.bind(this,chat)} key={index} className={this.state.active_chat == chat.id ? "chat_list active_chat" : "chat_list"}>
                                                    <div className="chat_people">
                                                        ...
                                                        <div className="chat_ib">
                                                            <h5>{chat.first_name} {chat.last_name}<span className="chat_date">{chat.last_active}</span></h5>
...
export default connect(mapStateToProps)(VendorMessages);