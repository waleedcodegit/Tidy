...
class CustomerMessages extends Component {
    ...
    componentWillMount(){
      setInterval(() => {
        Axios.post('/api/get_customer_chats').then(res=>{
            this.setState({
                chats:res.data,
            ...
            if(res.data.length > 0){
                Axios.post('/api/get_customer_chat_messages',{chat_id:this.state.active_chat == 0 ? res.data[0].id : this.state.active_chat}).then(res=>{
                    this.set_scroll();
                    console.log(Request)
                   if(res.data.status == 200){
    ...
    send_message(e){
        ...
        this.setState({
          ...
          Axios.post('/api/customer_message_sender',newmsg).then(res=>{
            ...
            Axios.post('/api/get_customer_chat_messages',{chat_id:this.state.active_chat}).then(res=>{
